...
{
	int i,k,k1,k2,y0;
	if((x1<0)||(x2>128)||(y1<0)||(y2>64)||(x1>x2))return;
	if(x1==x2)    //画竖线
	{
			for(i=0;i<(y2-y1);i++)
			{
...
void OLED_ShowChinese(uint8_t x,uint8_t y,uint8_t num,uint8_t size1,uint8_t n1)
{
	...
	while(size3--)
	{
		...
		n++;
			...
			{
				if((size1==16)&&(n1 == 1))
//						{temp=Wendu1[chr1][i];}//调用16*16字体
                                    {temp = weight1[chr1][i];}
                        else if((size1==16)&&(n1 == 2))
                                    {temp = weight2[chr1][i];}
				    else if((size1==16)&&(n1 == 3))
				                {temp = Wendu1[chr1][i];}
				    else if((size1==16)&&(n1 == 4))
				                {temp = ka[chr1][i];}
				else if(size1==24)
						{temp = CeWen[chr1][i];}//调用24*24字体
				else if(size1==32)       
...
void OLED_ScrollDisplay(uint8_t num,uint8_t space)
{
	...
	while(first)
	{
		if(m==0)
		{
	       OLED_ShowChinese(128,0,t,16,2); //写入一个汉字保存在OLED_GRAM[][]数组中
			t++;
		}
		if(t==num)
            {
